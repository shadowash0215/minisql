...
uint32_t Column::GetSerializedSize() const {
  // replace with your code here
  // return 0;
  return sizeof(uint32_t) * 4 + name_.length() + sizeof(TypeId) + sizeof(bool) * 2;
}
...
uint32_t Column::DeserializeFrom(char *buf, Column *&column) {
  ...
  TypeId type;
  uint32_t len, table_ind, SerializedSize = 0;
  bool nullable, unique;
  std::string column_name;
  COLUMN_MAGIC_NUM_R = MACH_READ_UINT32(buf);
  SerializedSize += sizeof(uint32_t);
  if (COLUMN_MAGIC_NUM_R != COLUMN_MAGIC_NUM) LOG(ERROR) << "magic number is wrong" << std::endl;
